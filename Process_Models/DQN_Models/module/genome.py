...
cut = np.zeros(4)  # blk_type (mol -> blk로 나눠지는 개수)
cut[0] = 506
cut[1] = 506
cut[2] = 400
cut[3] = 400

ratio = np.zeros((3, 4))  # month, blk_type (mol -> blk의 양품률)
ratio[0][0] = 0.851
ratio[1][0] = 0.851
ratio[2][0] = 0.851
...
time_len = submission_ini.shape[0]  # 총 시간

order = order_ini.copy()            # order 뒤에 0000을 추가한다
for i in range(35):
    order.loc[91+i, :] = ['0000-00-00', 0, 0, 0, 0]

# state가 정적이므로 (step의 결과에 영향을 받지 않으므로)
# 미리 모든 state를 계산해서 병렬적으로 처리한다
all_states = np.zeros((time_len + 1, 125))
for i in range(time_len + 1):
    all_states[i][0:124] = np.array(order.loc[i//24:(i//24+30), 'BLK_1':'BLK_4']).reshape(-1)
    all_states[i][124] = i % 24

# 시작시 prt, mol, blk의 stock
prt_2_stock = 258
...
class Genome():
    ...
    def predict(self):
        ...
        self.process_A = np.zeros((time_len, 4))
        self.process_B = np.zeros((time_len, 4))
        self.process_num = 0
        self.process_today_A = 0
        self.process_today_B = 0

        for s in range(time_len):
            self.update_mask()
            ...
            if out1_1 == 4:
                ...
                if (self.process_mode_1 == 1 and s >= 24 * 16 and self.process_num + out2_1 < prt_2_stock) or s >= 24*23:
            ...
            if out1_2 == 4:
                # 23일간은 mol2만 생산 가능
                if (self.process_mode_2 == 1 and s >= 24 * 16 and self.process_num + out2_2 < prt_2_stock) or s >= 24*23:
    ...
    # 인자로 받은 genome의 weights를 복사하여 설정한다
    ...
    # genome의 모델들의 weight를 list로 가져온다
    # list[list[np.ndarray]]
    def get_weights(self):
        return self.model1.get_weights(), self.model2.get_weights()

    # genome의 모델들의 weight를 설정한다
    # get_weights로 받아오는 타입과 동일해야 한다
